...
async def load_repository(request: LoadRepositoryRequest):
    ...
    try:
        ...
        fastcode_instance.load_repository(request.source, request.is_url, copy_to_workspace=request.copy_to_workspace)
...
async def checkout_commit_or_branch(request: CheckoutRequest):
    ...
    try:
        ...
        if request.commit_hash:
            ...
            success = fastcode_instance.checkout_commit(request.commit_hash)
        elif request.branch_name:
            logger.info(f"Checking out branch: {request.branch_name}")
            success = fastcode_instance.checkout_branch(request.branch_name)
